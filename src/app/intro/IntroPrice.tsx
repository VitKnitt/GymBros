...
export function IntroPrice() {
  const plans: Plan[] = [
    ...
    },
    {
      title: "1 měsíc",
      price: "1 200 Kč",
      features: ["Neomezený vstup", "Skupinové lekce", "Přístup 7 dní v týdnu"],
      image: cardsBg,
    },
    {
      title: "3 měsíce",
  ...
  return (
    ...
    >
      <article className="max-w-prose p-4 text-center">
        ...
        <p className="text-xl leading-relaxed text-five">
          Jak často na sobě makáš? To necháme na tobě. Čím pravidelněji
          trénuješ, tím víc výhod od nás získáš — a levněji.
        </p>
      </article>
        ...
        {plans.map((plan, index) => (
          <div
            ...
            className={`relative w-[280px] md:w-[320px] p-6 rounded-2xl overflow-hidden shadow-2xl text-white cursor-pointer transition-transform duration-500 hover:scale-105 ${
              plan.highlighted ? "border-4 border-two" : "border border-three"
            }`}
          >
            {/* Pozadí */}
      ...
      <p className="text-base leading-relaxed text-center max-w-prose">
        Registruj se online nebo přímo u nás v Gym-Bros Fitness. Chceš roční
        členství, firemní benefity nebo trénink na míru? Ozvi se nám — rádi vše
        nastavíme podle tebe.
      </p>
    </section>
  );
}
